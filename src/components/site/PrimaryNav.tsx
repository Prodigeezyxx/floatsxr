    ...
    >
      <div className="grid-container flex items-center justify-between h-[72px]">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {navLinks.map((link, index) => (
            <div
              ...
              onFocus={() => setOpenMega(link.label)}
              onBlur={(event) => {
            ...
            >
              <Link
                href={link.href}
                className={cn(
                  "flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-inkwell/72 transition-colors hover:bg-ecru/70 hover:text-inkwell"
                )}
                aria-haspopup={link.children ? "true" : undefined}
                aria-expanded={link.children ? openMega === link.label : undefined}
                aria-controls={link.children ? `nav-${link.label.toLowerCase().replaceAll(" ", "-")}` : undefined}
              >
                {link.label}
              ...
              {link.children && openMega === link.label && (
                <div
                  id={`nav-${link.label.toLowerCase().replaceAll(" ", "-")}`}
                  className={cn(
                    "absolute top-full grid gap-8 rounded-2xl border border-mist/40 bg-white/95 p-6 shadow-elevated backdrop-blur-xl",
                    link.children.length === 3 && "w-[720px] grid-cols-3",
                    link.children.length === 2 && "w-[520px] grid-cols-2",
                    link.children.length === 1 && "w-[280px] grid-cols-1",
                    index === 0 && "left-0",
                    index > 0 && index < navLinks.length - 2 && "left-1/2 -translate-x-1/2",
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <Link
            href="/get-a-demo"
            className="inline-flex min-h-10 items-center rounded-lg bg-cobalt px-5 text-sm font-medium text-white transition-all hover:-translate-y-0.5 hover:bg-cobalt/90 hover:shadow-glow"
          >
            Book a demo
          </Link>
        </div>
      ...
      {/* Mobile nav */}
      {mobileOpen && (
        <div id="mobile-navigation" className="space-y-1 border-t border-mist/30 bg-white px-6 pb-6 pt-2 lg:hidden">
          {navLinks.map((link) => (
            <div key={link.label}>
              <Link
                href={link.href}
          ...
          <div className="pt-4 space-y-3">
            ...
            <Link
              href="/get-a-demo"
              className="block rounded-lg bg-cobalt px-5 py-3 text-center text-sm font-medium text-white"
              onClick={() => setMobileOpen(false)}
            >
              Book a demo
            </Link>
