...
/** Every published brief, in the order they appear on /resources/case-studies/. */
...
/** Route of a single shareable brief. Trailing slash matches `trailingSlash: true`. */
export function caseStudyHref(id: string): string {
  return `/resources/case-studies/${id}/`;
}

/** Absolute URL of a brief, used for share targets, canonicals, and metadata. */
export function caseStudyShareUrl(id: string): string {
  return `${SITE_URL}${caseStudyHref(id)}`;
}
...
/** First paragraph of the brief, trimmed into a meta description. */
export function caseStudyDescription(study: CaseStudy, maxLength = 158): string {
  const source = study.brief[0] ?? study.title;
